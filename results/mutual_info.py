...
import argparse
import torch
from utils.save_load import LossRecorder, find_by_job_number, needed_remote_files
...
if __name__ == '__main__':
    ...
    args_from_file = '--job-dir {} --compare --prediction mean'.format(job_dir).split()
    ...
    for dir_name in os.scandir(args.job_dir):
        ...
        if '-' in dir_name.name:
            agg_type = 'cascad'
            sep = '-'
        elif '|' in dir_name.name:
            agg_type = 'parallel'
            sep = '|'

        jobs = [int(_) for _ in dir_name.name.split(sep)]
        ...
        if compare_with or prediction_method:
            ...
            with open('/tmp/files-{}'.format(dir_name.name), 'w') as f:
        ...
        print('*** {} *** {} ***'.format(dir_name.name, dataset))
