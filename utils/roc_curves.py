...
def roc_curve(ins, outs, *kept_tpr, two_sided=False, around='mean', validation=1000, debug=False):
    ...
    sorted_ins = np.sort(ins[ins_n_valid:])
    ins_ = np.asarray(ins)[:ins_n_valid]
...
if __name__ == '__main__':
    ...
    offset = 4
    for two_sided, factor in zip((False,) + (True,) * 10, (1,) + (1,) + (1,) * 10):
        print('2S' if two_sided else '1S')
        auroc, fpr, tpr, thr = roc_curve(factor * np.random.permutation(ins) + offset, outs, *kept_tpr,
