...
def scrisk(y_true, y_est, r_scores, g_scores, weight=0.5, n_tpr=1000):
    ...
    acc = (y_est == y_true).float().sum()/in_samples

    out_samples = len(y_true) - in_samples

    thresholds = torch.zeros(n_tpr)

    scores_ = {'g': g_scores, 'r': r_scores}
    i_ = {'in': y_true >= 0, 'out': y_true < 0}

    for i in i_:
        for s in scores_:
            _score = scores_[s][i_[i]]
    ...
    for i in range(n_tpr):
        ...
        # print('{:.1%} -- {:7.2e}'.format(checked_tpr[i], thresholds[i]))
    ...
    classif_errors_as_pos = i_pos & ((y_true >= 0) & (y_true != y_est)).unsqueeze(-1)

    tpr = i_true_pos.sum(0) / in_samples
    fpr = i_false_pos.sum(0) / out_samples
    selective_risk = classif_errors_as_pos.sum(0) / tpr / in_samples
...
if __name__ == '__main__':
    ...
    for j in jobs:
        ...
        for r, g in product(jobs[j]['r'], jobs[j]['g']):
            ...
            tpr, sr, fpr, acc = scrisk(y_true, y_est, r_scores, g_scores)
