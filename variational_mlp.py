...
class VariationalNetwork(Model):
    ...
    def get_data(self, source='mnist', *args, **kws):
    ...
    def directory_name(self):
        ...
        s = self.data['name'] + self.data['desc'] + '-'
        # s+= str(self.params['dim']) + '-'
        ls = self.params['layer_sizes']
        dl = self.params['decoded_layers']
        n = len(ls)
        
        layers_str = [str(ls[i]) + ('*' if i in dl else '') for i in range(n)]
        s+= '-'.join(layers_str)
    ...
    def get_intermediate_outputs(self, x=None, force=False):
        ...
        if not self.calculated_intermediate_outputs() or force or not x_train:
            for t in ts:
                print('getting output layer {}'.format(t))
                intermediate_layer_model = Model(inputs=main_net.input,
                                                 outputs=main_net.layers[t].output)
                outputs.append(intermediate_layer_model.predict(x))

            if x_train:
                self.data['intermediate_outputs'] = outputs
                self.params['var_trained'] = False
...
if __name__ == '__main__':
    ...
    dataset = 'mnist'
    dir_name = 'jobs/var-net-' + dataset

    try:
        var_net = VariationalNetwork.load_from_directory(dir_name)
    except (FileNotFoundError, OSError):
        print("Files not found -- creation of the network")
        var_net = VariationalNetwork()
        layer_sizes = [512]*0 +  [256] + [128] + [64] + [32, 16]
        test_data, test_labels = var_net.get_data(source=dataset)
        var_net.build_model(layer_sizes)
        var_net.build_var_dec()    

    test_data = var_net.data['x_test']
    test_labels = var_net.data['y_test']
    ...
    var_net.var_fit(var_epochs=var_epochs)

    t_test = var_net.get_intermediate_outputs(x=test_data)
